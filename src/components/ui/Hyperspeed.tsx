...
class CarLights {
  ...
  update(time: number) {
    if ((this.mesh.material as THREE.ShaderMaterial).uniforms.uTime) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    const appInstance = new App(container, options);
    appRef.current = appInstance;
    const bootstrap = async () => {
      try {
        await appInstance.loadAssets();
        if (!appInstance.disposed) {
          appInstance.init();
        }
      } catch (error) {
        console.error("Failed to initialise Hyperspeed", error);
        appInstance.init();
      }
    };
    bootstrap();
  ...
  return <div className="w-full h-full" ref={hyperspeedRef}></div>;
};
